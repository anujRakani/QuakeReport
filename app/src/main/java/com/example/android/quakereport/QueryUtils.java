...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONObject rootArray = new JSONObject(SAMPLE_JSON_RESPONSE);
            JSONArray featuresArray = rootArray.optJSONArray("features");

            for (int i = 0; i < featuresArray.length() ; i++ ) {
                JSONObject featureObj = featuresArray.getJSONObject(i);
                JSONObject propertiesObj = featureObj.getJSONObject("properties");

                magnitude = (float) propertiesObj.getDouble("mag");
